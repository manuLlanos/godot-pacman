...
# goes to a designated corner

@export var ghost: CharacterBody2D
@export var nav_agent: NavigationAgent2D
@export var scatter_target: Marker2D

func enter():
	nav_agent.target_position = scatter_target.global_position
	ghost.vulnerable = true
	ghost.can_attack = false
	ghost.speed = 0.7 * ghost.max_speed
	%Body.self_modulate = Color(0, 0, 1, 1)
